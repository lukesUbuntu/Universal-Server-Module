...
class UniversalServerModule extends Module {
		...
		if(!$this->sendNotification("service_notice_cancel", $service->fields, $package->module_row, null, $package->meta)) {
			$this->Input->setErrors(array(
				'service_notice_cancel' => array(
					'failed' => Language::_("UniversalServerModule.!error.service_notice_cancel.failed", true)
				)
			));
			return;
		}
	...
	public function suspendService($package, $service, $parent_package = null, $parent_service = null) {
		if(!$this->sendNotification("service_notice_suspend", $service->fields, $package->module_row, null, $package->meta)) {
			$this->Input->setErrors(array(
				'service_notice_suspend' => array(
					'failed' => Language::_("UniversalServerModule.!error.service_notice_suspend.failed", true)
				)
			));
			return;
		}
	...
	public function unsuspendService($package, $service, $parent_package = null, $parent_service = null) {
		if(!$this->sendNotification("service_notice_unsuspend", $service->fields, $package->module_row, null, $package->meta)) {
			$this->Input->setErrors(array(
				'service_notice_unsuspend' => array(
					'failed' => Language::_("UniversalServerModule.!error.service_notice_unsuspend.failed", true)
				)
			));
			return;
		}
	...
	public function renewService($package, $service, $parent_package = null, $parent_service = null) {
		if(!$this->sendNotification("service_notice_renew", $service->fields, $package->module_row, null, $package->meta)) {
			$this->Input->setErrors(array(
				'service_notice_renew' => array(
					'failed' => Language::_("UniversalServerModule.!error.service_notice_renew.failed", true)
				)
			));
			return;
		}
	...
	public function changeServicePackage($package_from, $package_to, $service, $parent_package = null, $parent_service = null) {
		if(!$this->sendNotification("service_notice_package_change", $service->fields, $package_to->module_row, null, $package_to->meta)) {
			$this->Input->setErrors(array(
				'service_notice_package_change' => array(
					'failed' => Language::_("UniversalServerModule.!error.service_notice_package_change.failed", true)
				)
			));
			return;
		}
	...
	public function manageAddRow(array &$vars) {
		...
		$this->view->set("required_options", array(
			'true' => Language::_("UniversalServerModule.true", true),
			'false' => Language::_("UniversalServerModule.false", true)
		));
		$this->view->set("encrypt_options", array(
			'true' => Language::_("UniversalServerModule.true", true),
			'false' => Language::_("UniversalServerModule.false", true)
		));
	...
	private function processPackage($type, array $vars, $package = null) {
		...
		if(!$this->sendNotification("package_notice_" . $type, $meta, $module_row_id, $vars)) {
			$this->Input->setErrors(array(
				'package_notice_' . $type => array(
					'failed' => Language::_("UniversalServerModule.!error.package_notice_" . $type . ".failed", true)
				)
			));
			return;
		}
	...
	private function processService($type, array $vars, $package = null) {
		...
		if(isset($vars['use_module']) && $vars['use_module'] == "true") {
			if(!$this->sendNotification("service_notice_" . $type, $meta, $module_row_id, $vars, $package->meta)) {
				$this->Input->setErrors(array(
					'service_notice_' . $type => array(
						'failed' => Language::_("UniversalServerModule.!error.service_notice_" . $type . ".failed", true)
					)
				));
				return;
			}
		...
		foreach($field_data as $field) {
			...
			switch($field['type']) {
				...
				case "textarea":
					...
					$label->attach($fields->{$field_type}($field_name, $field_value, array(
						'id' => "uni_" . $field['name']
					)));
				...
				case "password":
					$label = $fields->label($field['label'], "uni_" . $field['name']);
					$label->attach($fields->{$field_type}($field_name, array(
						'id' => "uni_" . $field['name'],
						'value' => $field_value
					)));
					$fields->setField($label);
				...
				case "select":
					$label = $fields->label($field['label'], "uni_" . $field['name']);
					$label->attach($fields->{$field_type}($field_name, $options, $field_value, array(
						'id' => "uni_" . $field['name']
					)));
					$fields->setField($label);
					break;
				...
				case "checkbox":
					...
					foreach($options as $key => $value) {
						...
						$label->attach($fields->{$field_type}($field_name . ($field['type'] == "checkbox" ? "[]" : ""), $key, $checked, array(
	...
	private function sendEmailNotice($action, $tags, $meta) {
		...
		if(strpos($action, "service") !== false) {
			...
			$body    = array(
				'text' => $meta->service_email_text,
				'html' => $meta->service_email_html
			);
		} else {
			$from    = $meta->package_email_from;
			$subject = $meta->package_email_subject;
			$body    = array(
				'text' => $meta->package_email_text,
				'html' => $meta->package_email_html
			);
		}
	...
	public function moreinfo($package, $service, array $get = null, array $post = null, array $files = null) {
		...
		foreach($servicefields as $field) {
			if(preg_match("/ded_ips/i", $field['key'])) {
				$ded_ip = $field['value'];
				$ded_ip = explode("-", $ded_ip);
				$ded_ip = trim($ded_ip[0]);
			}
			if(preg_match("/root_pass/i", $field['key'])) {
				$root_pass = $field['value'];
			}
			if(preg_match("/control_url/i", $field['key'])) {
				$control_url = $field['value'];
			}
			if(preg_match("/prod_id/i", $field['key'])) {
				$prod_id = $field['value'];
			}
		}
		//SSH Access Data
		...
		if($_GET['on'] == 'true') {
			if(!empty($row->meta->service_notice_on_button)) {
				if(!$this->sendNotification("service_notice_on_button", $service->fields, $package->module_row, null, $package->meta)) {
					$this->Input->setErrors(array(
						'service_notice_on_button' => array(
							'failed' => Language::_("UniversalServerModule.server_on_error", true)
						)
					));
					$server_on_exe = false;
				} else {
		...
		if($_GET['off'] == 'true') {
			if(!empty($row->meta->service_notice_off_button)) {
				if(!$this->sendNotification("service_notice_off_button", $service->fields, $package->module_row, null, $package->meta)) {
					$this->Input->setErrors(array(
						'service_notice_off_button' => array(
							'failed' => Language::_("UniversalServerModule.server_off_error", true)
						)
					));
		...
		if($_GET['reset'] == 'true') {
			if(!empty($row->meta->service_notice_reset_button)) {
				if(!$this->sendNotification("service_notice_reset_button", $service->fields, $package->module_row, null, $package->meta)) {
					$this->Input->setErrors(array(
						'service_notice_reset_button' => array(
							'failed' => Language::_("UniversalServerModule.server_reset_error", true)
						)
					));
		...
		//cPanel
		if(ping($ded_ip, 2082, 1) && $cpanel == 'Without Panel') {
			$cpanel = 'cPanel';
			if(empty($control_url)) {
				$control_url = 'https://' . $ded_ip . ':2083';
			}
		}
		//DirectAdmin
		if(ping($ded_ip, 2222, 1) && $cpanel == 'Without Panel') {
			$cpanel = 'DirectAdmin';
			if(empty($control_url)) {
				$control_url = 'http://' . $ded_ip . ':2222';
			}
		}
		//Webuzo
		if(ping($ded_ip, 2002, 1) && $cpanel == 'Without Panel') {
			$cpanel = 'Webuzo';
			if(empty($control_url)) {
				$control_url = 'http://' . $ded_ip . ':2002';
			}
		}
		//Kloxo-MR
		if(ping($ded_ip, 7777, 1) && $cpanel == 'Without Panel') {
			$cpanel = 'Kloxo-MR';
			if(empty($control_url)) {
				$control_url = 'https://' . $ded_ip . ':7777';
			}
		}
		//VestaCP
		if(ping($ded_ip, 8083, 1) && $cpanel == 'Without Panel') {
			$cpanel = 'Vesta Control Panel';
			if(empty($control_url)) {
				$control_url = 'https://' . $ded_ip . ':8083';
			}
		}
		//CWP
		if(ping($ded_ip, 2030, 1) && $cpanel == 'Without Panel') {
			$cpanel = 'CentOS Web Panel';
			if(empty($control_url)) {
				$control_url = 'http://' . $ded_ip . ':2030';
			}
		}
		//SolusVM
		if(ping($ded_ip, 5353, 1) && $cpanel == 'Without Panel') {
			$cpanel = 'SolusVM';
			if(empty($control_url)) {
				$control_url = 'https://' . $ded_ip . ':5656/admincp';
			}
		}
		//Get Status Server 
		if(serverIsUp($ded_ip) || ping($ded_ip, 80, 1)) {
			$status_ded    = Language::_("UniversalServerModule.active", true);
			$color_ded_sta = '#008000';
		} else {
			$status_ded    = Language::_("UniversalServerModule.down", true);
			$color_ded_sta = '#800000';
		}
		//Get Screenshoot
		if(ping($ded_ip, 80, 1)) {
		...
		$this->view->set("row->meta->service_code_on_button", $row->meta->service_code_on_button);
		$this->view->set("row->meta->service_code_reset_button", $row->meta->service_code_reset_button);
		$this->view->set("row->meta->service_code_off_button", $row->meta->service_code_off_button);
	...
	public function colocation($package, $service, array $get = null, array $post = null, array $files = null) {
		...
		foreach($servicefields as $field) {
			...
			if(preg_match("/ded_hostname/i", $field['key'])) {
				$ded_hostname = $field['value'];
			}
			if(preg_match("/colo_u/i", $field['key'])) {
				$colo_u = $field['value'];
			}
			if(preg_match("/colo_pos/i", $field['key'])) {
				$colo_pos = $field['value'];
			}
			if(preg_match("/colo_weight/i", $field['key'])) {
				$colo_weight = floatval($field['value']);
			}
			if(preg_match("/colo_datacenter/i", $field['key'])) {
				$colo_datacenter = $field['value'];
			}
			if(preg_match("/colo_floor/i", $field['key'])) {
				$colo_floor = $field['value'];
			}
			if(preg_match("/colo_room/i", $field['key'])) {
				$colo_room = $field['value'];
			}
			if(preg_match("/colo_rack/i", $field['key'])) {
				$colo_rack = $field['value'];
			}
		}
	 ...
	 * @return string The string representing the contents of this tab
	 */
	public function bandwidth($package, $service, array $get = null, array $post = null, array $files = null) {
		$row           = $this->getModuleRow($package->module_row);
		$servicefields = array();
		foreach($row->meta as $key => $value) {
			if(strpos($key, "service_field_name") !== false) {
		...
		//Connect to ServerStadium API
		if(!empty($prod_id)) {
			$postfields = array(
				'api_key' => '36C37223D7031236A0931FAF49E18745', //ServerStadium API Key
				'cust_email' => 'admin@cyandark.com', //ServerStadium Customer Email
				'cust_id' => 4394,
				'cust_ip' => $_SERVER['SERVER_ADDR'],
				'prod_id' => $prod_id,
				'action' => 'bandwidth'
			);
			$ch         = curl_init();
			curl_setopt($ch, CURLOPT_URL, 'https://api.wowrack.com/');
		...
		$this->view           = new View("bandwidth", "default");
		$this->view->base_uri = $this->base_uri;
		// Load the helpers required for this view
		Loader::loadHelpers($this, array(
			"Form",
		...
		$this->view->set("service_id", $service->id);
		$this->view->set("this", $this);
		$this->view->set("arr", $arr);
		$this->view->set("prod_id", $prod_id);
		$this->view->set("view", $this->view->view);
		$this->view->setDefaultView("components" . DS . "modules" . DS . "universal_server_module" . DS);
		if(!empty($prod_id)) {
			return $this->view->fetch();
		} else {
			return "<h4>" . Language::_("UniversalServerModule.bandwidth", true) . "</h4><br><p>" . Language::_("UniversalServerModule.!error.service_notice_edit.failed", true) . "</p>";
		}
	}
	/**
	 * Server Options
	...
	public function serveroptions($package, $service, array $get = null, array $post = null, array $files = null) {
		...
		foreach($servicefields as $field) {
			...
			if(preg_match("/ded_os/i", $field['key'])) {
				$ded_os = $field['value'];
			}
		}
		//SSH Access Data
		...
		$user   = 'root';
		$pass   = $root_pass;
		if(preg_match("/centos/i", $ded_os) || preg_match("/fedora/i", $ded_os) || preg_match("/scientific/i", $ded_os) || preg_match("/cloudlinux/i", $ded_os)) {
			$os = 'rhel';
		} else {
			if(preg_match("/freebsd/i", $ded_os)) {
				$os = 'bsd';
			} else {
				$os = 'debian';
			}
		...
		if(ItsSshActivated($server, $port, $user, $pass)) {
			...
			if(isset($_GET['update'])) {
				$result = upgradeSoftwareSSH($server, $port, $user, $pass, $os);
			}
			//Clean Software
			if(isset($_GET['clean'])) {
				$result = cleanSoftwareSSH($server, $port, $user, $pass, $os);
			}
			...
			if(!empty($post)) {
				if($post['password'] == $post['confirm_password'] && !empty($post['password'])) {
					...
					if($service->fields[7]->key == 'root_pass') {
						$service->fields[7]->value = $post['password'];
					} else {
						$service->fields[8]->value = $post['password'];
					}
					$this->sendNotification("service_notice_edit", $service->fields, $package->module_row, null, $package->meta);
				}
				if(!empty($post['hostname'])) {
					$result                    = changeServerHostnameSSH($server, $port, $user, $pass, $post['hostname']);
					$service->fields[2]->value = $post['hostname'];
					$this->sendNotification("service_notice_edit", $service->fields, $package->module_row, null, $package->meta);
				}
			}
	...
	public function getClientTabs($package) {
		...
		if((preg_match("/dedicated/i", $var_dump) && preg_match("/server/i", $var_dump) && !preg_match("/colocation/i", $var_dump)) || (preg_match("/dedicated/i", $var_dump) && preg_match("/servers/i", $var_dump) && !preg_match("/colocation/i", $var_dump))) {
			return array(
				'moreinfo' => array(
					'name' => Language::_("UniversalServerModule.moreinfo", true),
					'icon' => "fa fa-gears"
				),
				'ssh_console' => array(
					'name' => Language::_("UniversalServerModule.ssh_console", true),
					'icon' => "fa fa-terminal"
				),
				'ftp_client' => array(
					'name' => Language::_("UniversalServerModule.ftp_client", true),
					'icon' => "fa fa-dashboard"
				),
				'stats' => array(
					'name' => Language::_("UniversalServerModule.server_stats", true),
					'icon' => "fa fa-area-chart"
				),
				'bandwidth' => array(
					'name' => Language::_("UniversalServerModule.bandwidth", true),
					'icon' => "fa fa-sitemap"
				),
				'serveroptions' => array(
					'name' => Language::_("UniversalServerModule.add_row.service_title", true),
					'icon' => "fa fa-cloud"
				)
			);
		} else {
			return array(
				...
				'colocation' => array(
					'name' => Language::_("UniversalServerModule.colocation", true),
					'icon' => "fa fa-building"
				)
			);
		}
